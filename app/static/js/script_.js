...
async function startJob(fileId) {
  let card = document.getElementById("file-" + fileId)
  let quality = card.querySelector(".quality-select")?.value || 'best'
...
async function pollJobs() {
  ...
  try {
    ...
    for (const j of data.jobs) {
      jobs[j.id] = new APIJob(j);
      _index_file_job[j.file_id] = j.id
    }
    for (const f of data.files) {
      file = files[f.id] = new APIFile(f);
    
      let job = jobs[_index_file_job[f.id]];
      file.status = 'queued';
      if (job) {
        if (job.error) file.status = 'error';
        else file.status = 'downloading';
    ...
    console.log(Object.values(jobs)[0]);
...
function renderFiles() {
  ...
  const current = new Set(list.map(f => 'file-' + f.id));
  [...container.children].forEach(el => { if (!current.has(el.id)) el.remove(); });

  // Create missing cards
  list.forEach(f => {
    if (!document.getElementById('file-' + f.id)) {
      container.appendChild(createFileCard(f));
    }
    updateJobCard(document.getElementById('file-' + f.id), f);
  });
...
function createFileCard(file) {
  ...
  const isDone = file.downloaded;
  const thumbSrc = esc(thumbUrl(file.thumbnail));
  const thumb = file.thumbnail
    ? `<div style="position:relative;width:100%;height:160px;overflow:hidden;flex-shrink:0"${isDone ? ` onclick="openPlayer('${file.id}')" style="position:relative;width:100%;height:160px;overflow:hidden;flex-shrink:0;cursor:pointer"` : ''}>
    ...
    : `<div style="width:100%;height:160px;background:var(--bg);display:flex;align-items:center;justify-content:center;font-size:32px;color:var(--text3)"${isDone ? ` onclick="openPlayer('${file.id}')" style="cursor:pointer"` : ''}>🎬</div>`;
  ...
  card.innerHTML = `
    <div class="job-inner">
      ...
      <div class="job-body">
        <div class="job-info">
          <div class="job-title">${esc(file.title || file.source)}</div>
          <div class="job-meta">
            <span class="job-site">${siteBadgeHtml(file.site, file.source)}</span>
    ...
    <div class="job-footer">
      ...
      <div class="job-actions" style="flex-wrap:wrap;gap:5px">
        ...
        <button class="icon-btn" title="Copy link" onclick="copyJobLink('${file.id}')">🔗</button>
        <button class="icon-btn del" title="Remove" onclick="deleteJob('${file.id}')">✕</button>
      </div>
    </div>`;
  return card;
}
 ...
 * @param {APIFile} file 
 */
function updateJobCard(card, file) {
  const isMissing = file.downloaded && missingFiles.has(file.id);

  let job = jobs[_index_file_job[file.id]]

  card.className = `job-card status-${file.status}${isMissing ? ' file-missing' : ''}`;

  const badge = card.querySelector('.job-badge');
  if (badge) {
    badge.className = `status-badge badge-${file.status} job-badge`;
  ...
  if (metaRight) {
    if (file.status === 'downloading') {
      const speedHtml = file.speed ? `<span style="color:var(--accent);font-weight:500">${esc(job.speed)}</span>` : '';
      const etaHtml = file.eta ? `<span style="color:var(--text2)">ETA <span style="color:var(--accent2)">${esc(job.eta)}</span></span>` : '';
      ...
      metaRight.innerHTML = `<span style="color:var(--accent3)">${esc((job.error || '').slice(0, 60))}</span>`;
    } else {
      metaRight.textContent = file.quality || '';
    }
  }

  const qualitySlot = card.querySelector('.job-quality-slot');
  if (qualitySlot && (file.status === 'queued' || file.status === 'error')) {
    const opts = file.quality_options
      .map(q => `<option value="${esc(q)}"${q === file.quality ? ' selected' : ''}>${esc(q)}</option>`)
      .join('');
    ...
    if (!qualitySlot.querySelector('select')) {
      qualitySlot.innerHTML = `<select class="quality-select" style="font-size:10px;padding:4px 6px;height:28px"
        onchange="changeQuality('${file.id}', this.value)">${opts}</select>`;
    } else {
      qualitySlot.querySelector('select').value = file.quality || 'best';
    }
  } else if (qualitySlot) {
  ...
  if (actionSlot) {
    if (file.status === 'queued' || file.status === 'error') {
      if (!actionSlot.querySelector('.start')) {
        actionSlot.innerHTML = `<button class="icon-btn start" title="Start download" onclick="startJob('${file.id}')">▶</button>`;
      }
    } else {
  ...
  if (revealSlot) {
    if (isMissing) {
      if (!revealSlot.querySelector('.redownload')) {
        revealSlot.innerHTML = `<button class="icon-btn redownload" title="File missing — redownload" onclick="redownloadAndStart('${file.id}')">↺</button>`;
      }
    } else if (file.downloaded) {
      if (!revealSlot.querySelector('.reveal')) {
        revealSlot.innerHTML = `<button class="icon-btn reveal" title="Show in Explorer" onclick="revealInExplorer('${file.id}')">📂</button>`;
      }
    } else {
  ...
  if (progressWrap && progressFill) {
    const show = file.status === 'downloading' || file.status === 'done' || file.status === 'error';
    progressWrap.style.display = show ? 'block' : 'none';
    progressFill.style.width = ((job || {}).progress || file.status == 'error' ? 100 : 0) + '%'; /// FIX ?
    progressFill.className = 'progress-bar-fill' +
      (file.status === 'done' ? ' done' : file.status === 'error' ? ' error' : '');
  }
  // Show progress pct in meta when downloading
  const pctSpan = card.querySelector('.job-pct');
  if (file.status === 'downloading' && job.progress > 0) {
    if (!pctSpan) {
      const badge = card.querySelector('.job-badge');
      if (badge) {
    ...
    if (card.querySelector('.job-pct')) card.querySelector('.job-pct').textContent = job.progress + '%';
  } else if (pctSpan) {
    pctSpan.remove();
  }
}
...
(async () => {
  try {
    ...
    if (Object.keys(jobs).length) startPolling()
    renderFiles();
